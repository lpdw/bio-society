...
const bodyVerificator = (req, res, next) => {
    if (req.body.username && req.body.password && req.body.firstname && req.body.lastname && req.body.address && req.body.postcode && req.body.phone_number) {
       return next();
    }

    const attributes = _.keys(req.body);
    const mandatoryAttributes = ['username', 'password', 'firstname', 'lastname', 'address', 'postcode', 'phone_number'];
